...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    // (old) RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y - 8.0f));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    // Draw frame (custom)
    const ImU32 frame_col = IM_COL32(16, 16, 16, 255);
    RenderNavHighlight(frame_bb, id);

    //border
    RenderFrame(ImVec2(frame_bb.Min.x -1, frame_bb.Min.y - 1), ImVec2(frame_bb.Max.x + 1, frame_bb.Max.y + 1), IM_COL32(8,8,8,255), true, g.Style.GrabRounding);
   ...
   // ImVec4 grab_color = ColorConvertU32ToFloat4(GetColorU32(ImGuiCol_SliderGrab));

    // Render grab (custom) 
    if (grab_bb.Max.x > grab_bb.Min.x) {
     // window->DrawList->AddRectFilled(grab_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
     // window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y), ImVec2(grab_bb.Max.x + 2, grab_bb.Max.y + 2), GetColorU32(ImGuiCol_SliderGrab), style.GrabRounding);
    ...
    RenderText(ImVec2(grab_bb.Max.x - IM_ARRAYSIZE(label), grab_bb.Max.y - 5), value_buf, value_buf_end);
    //RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.5f));

    if (label_size.x > 0.0f)
        RenderText(ImVec2(grab_bb.Max.x, frame_bb.Min.y + style.FramePadding.y), label);
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    window->DrawList->AddRectFilledMultiColor(ImVec2(bb.Min.x, bb.Min.y + 16), ImVec2(bb.Max.x - 2, bb.Max.y + 4),
        tab_col,
        tab_col,
        ImGui::ColorConvertFloat4ToU32(tab_col_vec4_darker),
        ImGui::ColorConvertFloat4ToU32(tab_col_vec4_darker)
        );
    RenderNavHighlight(ImRect(ImVec2(bb.Min.x - 2, bb.Min.y + 16), ImVec2(bb.Max.x + 2, bb.Max.y + 6)), id);
