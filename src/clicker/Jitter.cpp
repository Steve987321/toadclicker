...
/*
      std::thread t([&]()
          {
              while (pt.x != dst.x && pt.y != dst.y)
              {
                  pt.x < dst.x ? pt.x++ : pt.x == dst.x ? pt.x = dst.x : pt.x--;
                  pt.y < dst.y ? pt.y++ : pt.y == dst.y ? pt.y = dst.y : pt.x--;
                  ...
                  if (toad::jitter::intensity_X > toad::jitter::intensity_Y && i > toad::jitter::intensity_X)
                  {
                      pt.x = dst.x;
                      pt.y = dst.y;
                  }
                  else if (toad::jitter::intensity_Y > toad::jitter::intensity_X && i > toad::jitter::intensity_Y)
                  {
                      pt.x = dst.x;
                      pt.y = dst.y;
                  }
